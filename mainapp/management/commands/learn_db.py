...
class Command(BaseCommand):
    ...
    for orderitem in test_orderss:
       print(f'{orderitem.action_order:2}: заказ №{orderitem.pk:3}: {orderitem.product.name:15}: скидка{abs(orderitem.total_price):6.2f} руб. | {orderitem.order.updated_at - orderitem.order.created_at}')
